...
export async function generateMetadata({ params, searchParams }) {
  ...
  return {
      title: `birdseyereview ${id.replaceAll('_', ' ')}`
  }
}
...
export default function Projects(props) {
  ...
  return (
    <ProjectLayout title="bird's eye review">
      <article id="article">
        <h1 style={{ marginBottom: "0", textAlign: "center" }}>{post.data.title}</h1>
        <h5 style={{ marginTop: "0", textAlign: "center" }}>{post.data.desc}</h5>
        <div style={{ marginTop: "20px" }}>{parse(result)}</div>
      </article>
    </ProjectLayout>
  );
}
